...
public class AdbUtility {
    ...
    public static void restore(String source) throws Exception {
        ...
        if (!f.exists()) {
            throw new AdbException(source + " : Not found");
        }
    ...
    public static void push(String source, String destination, boolean log) throws Exception {
        ...
        if (log) {
            logger.info("Pushing " + f.getAbsolutePath() + " to " + destination);
        }
        else {
            logger.debug("Pushing " + f.getAbsolutePath() + " to " + destination);
        }
    ...
    public static String getBusyboxVersion(String path) {
        try {
            ...
            if (sc.hasNextLine()) {
                String line = sc.nextLine();
                if (line.contains("BusyBox v1") && line.contains("multi-call")) {
                    return line;
                }
            }
            return "";
    ...
    public static String getStatus() {
        try {
            ...
            if (command.getStdErr().contains("unauthorized")) {
                return "adb_unauthorized";
            }
            if (command.getStdErr().contains("error")) {
                return "normal";
            }
            if (command.getStdOut().contains("device")) {
                return "adb";
            }
            return "none";
        }
    ...
    public static long rawBackup(String source, String destination) {
        ...
        if (!result.contains("bytes transferred")) {
            return 0L;
        }
