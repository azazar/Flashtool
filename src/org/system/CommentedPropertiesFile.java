...
public class CommentedPropertiesFile extends java.util.Properties {
    ...
    public void store(OutputStream out, String header) throws IOException {
        ...
        while (i.hasNext()) {
            ...
            else if (commented.containsKey(line)) {
                writer.println("#" + line.substring(2) + "=" + commented.getProperty(line));
            }
            else {
                writer.println(line);
            }
    ...
    public void updateProperty(String key, String value) {
        if (isCommented(key) && !isNotCommented(key)) {
            unComment(key);
        }
    ...
    public void updateCommentedProperty(String key, String value) {
        if (isNotCommented(key) && !isCommented(key)) {
            comment(key);
        }
    ...
    public void setCommentedProperty(String key, String value) {
        ...
        if (lines.indexOf("_c" + key) < 0) {
            lines.add("_c" + key);
        }
    }

    public Object setProperty(String key, String value) {
        if (lines.indexOf(key) < 0) {
            lines.add(key);
        }
