...
public class Parser {
    ...
    public static Session parse(String usblog, String extractedsin) throws Exception {
        ...
        while (usbStream.hasAvailableData()) {
            ...
            if (rec.header.usb_UsbDeviceHandle == 0) {
                continue;
            }
            if (rec.getDataString().contains("getvar") && s1parser == true) {
                s1parser = false;
            }
            if (s1parser == false) {
        ...
        while (ifiles.hasNext()) {
            try {
                ...
                if (sinfile.getVersion() != 4) {
                    ...
                    if (Arrays.equals(source, sinfile.getHeader())) {
                        return sinfile.getFile().getName();
                    }
                }
            }
