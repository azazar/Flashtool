...
public class SinParser {
    ...
    public void parseHash(JBBPBitInputStream sinStream) throws IOException {
        ...
        if (hashLen > 0) {
            ...
            if (blocks.block.length == 1 && blocks.block[0].offset != 0) {
    ...
    public String getDataTypePriv(byte[] res) throws IOException {
        if (BytesUtil.startsWith(res, new byte[]{0x7F, 0x45, 0x4C, 0x46})) {
            return "elf";
        }
        if (BytesUtil.startsWith(res, new byte[]{0x03, 0x00, 0x00, 0x00, 0x01, 0x00, 0x00, 0x00, (byte) 0xFF, (byte) 0xFF, 0x00, 0x00, 0x00, 0x00, 0x00, 0x00})) {
            return "yaffs2";
        }
