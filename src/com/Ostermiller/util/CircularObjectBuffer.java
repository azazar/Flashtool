...
public class CircularObjectBuffer<ElementType> {
     ...
     * empty buffer will have the readPostion and the writePosition equal to
     * each other. A full buffer will have the writePosition one less than the
     * readPostion.
     * <p>
     * There are two important indexes into the buffer: The readPosition, and
     * the writePosition. The Objects available to be read go from the
     * readPosition to the writePosition, wrapping around the end of the buffer.
     * The space available for writing goes from the write position to one less
     * than the readPosition, wrapping around the end of the buffer.
     *
     * @since ostermillerutils 1.00.00
     */
     ...
     * from the buffer may return null if the buffer is empty, otherwise a read
     ...
     * <p>
     * Note that the number of Objects available plus the number of Objects free
     * may not add up to the capacity of this buffer, as the buffer may reserve
     ...
     *
     * @param blockingWrite true writing to a full buffer should block until
     * space is available, false if an exception should be thrown instead.
     *
     * @since ostermillerutils 1.00.00
     */
    ...
    /**
     * Get Objects into an array from this buffer. This method should be called
     * by the consumer. This method will block until some input is available, or
    ...
    /**
     * Get Objects into a portion of an array from this buffer. This method
     * should be called by the consumer. This method will block until some input
    ...
    /**
     * This method should be used by the producer to signal to the consumer that
     * the producer is done producing objects and that the consumer should stop
     * asking for objects once it has used up buffered objects.
     * <p>
     * Once the producer has signaled that it is done, further write()
     * invocations will cause an IllegalStateException to be thrown. Calling
     * done() multiple times, however, has no effect.
     *
     * @since ostermillerutils 1.00.00
     */
    public void done() {
    ...
    /**
     * Fill this buffer with array of Objects. This method should be called by
     * the producer. If the buffer allows blocking writes, this method will
     * block until all the data has been written rather than throw a
     * BufferOverflowException.
     *
     * @param buf Array of Objects to be written
     * @throws BufferOverflowException if buffer does not allow blocking writes
     * and the buffer is full. If the exception is thrown, no data will have
    ...
    /**
     * Fill this buffer with a portion of an array of Objects. This method
     * should be called by the producer. If the buffer allows blocking writes,
     * this method will block until all the data has been written rather than
     ...
     * producer. If the buffer allows blocking writes, this method will block
